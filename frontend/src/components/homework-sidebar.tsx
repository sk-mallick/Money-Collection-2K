...
import { BarChart3, ClipboardCheck, Download, LogOut, ChevronsUpDown, Moon, Sun, LayoutGrid } from 'lucide-react';
import { useTheme } from '@/components/theme-provider';

const navItems = [
  { title: 'Dashboard', icon: BarChart3, path: '/homework/dashboard' },
  { title: 'Record Entry', icon: ClipboardCheck, path: '/homework/record' },
  { title: 'Monthly Reports', icon: Download, path: '/homework/reports' },
];
        ...
        <SidebarMenu>
          <SidebarMenuItem>
            <SidebarMenuButton size="lg" className="hover:bg-transparent cursor-default p-0! group-data-[collapsible=icon]:p-0!">
              <div className="flex items-center justify-center size-8 shrink-0 rounded-lg shadow-md bg-gradient-to-br from-amber-500 to-orange-600 aspect-square">
                <ClipboardCheck className="size-4.5 text-white shrink-0" strokeWidth={2} />
              </div>
              <div className="flex-1 text-left min-w-0 overflow-hidden group-data-[collapsible=icon]:hidden">
                <span className="font-bold text-sm text-foreground tracking-wider truncate block">Homework</span>
              </div>
            </SidebarMenuButton>
          </SidebarMenuItem>
        </SidebarMenu>
            ...
            <SidebarMenu>
              {navItems.map(item => {
                const isActive = location.pathname === item.path ||
                  (item.path !== '/homework/dashboard' && location.pathname.startsWith(item.path));
                return (
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <SidebarMenuButton size="lg" className="data-[state=open]:bg-sidebar-accent">
                  <Avatar className="h-10 w-10 md:h-8 md:w-8">
                    <AvatarFallback className="bg-gradient-to-br from-amber-500 to-orange-600 text-white text-sm md:text-xs font-bold">
              ...
              <DropdownMenuContent className="w-56" side="top" align="start" sideOffset={4}>
                <div className="flex items-center gap-2 p-2">
                  <Avatar className="h-10 w-10 md:h-8 md:w-8">
                    <AvatarFallback className="bg-gradient-to-br from-amber-500 to-orange-600 text-white text-xs md:text-[10px] font-bold">
                      {(user?.name || 'A').charAt(0).toUpperCase()}
                    </AvatarFallback>
                  </Avatar>
